...
# Boot50 model variables and coefficients  
boot50_models <- lapply(results, function(x) x$coefficients_table[Cs(variable, coef_boot50_model)])  %>%  reduce(left_join, by = "variable")
colnames(boot50_models) <- col_names

# Shrunken coefficients (from boot50 model) 
...
### AUC from full and boot50 models

```{r}
# AUCs
auc_full <- lapply(results, function(x) x$c_statistics[Cs(full_model, boot50_model)]) 
auc_full                                 
```
...
addWorksheet(imputation_results, "Boot50 model coefficients")
addWorksheet(imputation_results, "Boot50-shrink coefficients")
...
writeData(imputation_results, sheet = "Boot50 model coefficients", x = boot50_models)
writeData(imputation_results, sheet = "Boot50-shrink coefficients", x = shrink_models)
