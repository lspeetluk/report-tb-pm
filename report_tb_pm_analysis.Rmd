...
#load("/Users/Lauren/Box/RePORT Brazil/Datasets/Save/tb_pm_analysis_files")
```
...
# Calibration plot
plot(calibrate(boot_model_50, B=200), xlab="Predicted Probability of Unsuccessful Outcome", ylab="Actual Proability of Unsuccessful Outcome", cex.lab=1.4, cex.axis=1.1)
```
...
# Calibration plot
plot(calibrate(boot_model_60, B=200), xlab="Predicted Probability of Unsuccessful Outcome", ylab="Actual Proability of Unsuccessful Outcome", cex.lab=1.4, cex.axis=1.1)
```
...
# Calibration plot
plot(calibrate(boot_model_70, B=200), xlab="Predicted Probability of Unsuccessful Outcome", ylab="Actual Proability of Unsuccessful Outcome", cex.lab=1.4, cex.axis=1.1)
```
...
# Calibration plot
plot(calibrate(boot_model_90, B=200), xlab="Predicted Probability of Unsuccessful Outcome", ylab="Actual Proability of Unsuccessful Outcome", cex.lab=1.4, cex.axis=1.1)
```
...
jpeg(here("images", "cal_plot_boot.jpeg"), width=500, height=500)
plot(calibrate(boot_model, B=200), xlab="Predicted Probability of Unsuccessful Outcome", ylab="Actual Proability of Unsuccessful Outcome", cex.lab=1.4, cex.axis=1.4)
dev.off()
...
We see that the variables through age can predict the full model with an R2 of 0.986 (this is the same as the 70% model)
...
boot_model <- Newlabels(boot_model, c("Hemoglobin (g/dL)", "HIV-positive", "Drug use", "Diabetes", "Age group", "Education (years)", "Tobacco use"))
...
```{r}
save.image(file="/Users/Lauren/Box/RePORT Brazil/Datasets/Save/tb_pm_analysis_files")
```
...
mi_sum2 %>% 
  tbl_cross(nat2_cat, hiv,
            percent="col")

mi_sum2 %>%
  select(nat2_cat, hiv, unsuccessful, outcome_cat) %>%
  tbl_summary(by="nat2_cat", type=list(c(hiv, unsuccessful, outcome_cat) ~ "categorical"), percent="row") %>% 
  add_p()
```
...
```{r}
base_model <- boot_model

# ---- Outcome -----
y = mi_sum2$unsuccessful
```


```{r}
add_nat2_form <- as.formula(paste0("unsuccessful ~", (paste(boot_vars, collapse = "+")), "+ nat2_cat"))

add_nat2 <-  lrm(add_nat2_form, data=mi_sum2) 
...
```{r}
add_nat2_form_int <- as.formula(paste0("unsuccessful ~", (paste(boot_vars, collapse = "+")), "+ nat2_cat + nat2_cat*hiv"))
add_nat2_int <-  lrm(add_nat2_form_int, data=mi_sum2)
add_nat2_int

with(mi_sum2, table(nat2_cat, hiv, unsuccessful))

# Added value of interaction
lrtest(add_nat2_int, add_nat2)
```

No interaction between NAT2 and HIV
...
data_with_missing <- analysis_data %>% 
  mutate(
    ...
    cd4_lt200_all = fct_case_when( # AIDS = CD4 < 200
       ...
       cd4_count <200 ~ "CD4 <200"),
    vl_all = ifelse(is.na(viral_load), 0, viral_load), 
...
data_with_missing %>% 
  filter(hiv==1) %>% 
  select(all_of(hiv_vars), cd4_lt200_all, vl_gt200_all, vl_all, unsuccessful) %>% 
  tbl_summary(by="unsuccessful") %>% 
  add_overall()
...
```{r}
boot_vars_no_hiv <- boot_vars[!boot_vars %in% "hiv"]

add_art_form <- as.formula(paste0("unsuccessful ~", (paste(boot_vars_no_hiv, collapse = "+")), "+ baseline_art_all"))

add_art <-  lrm(add_art_form, data=data_to_analyze) 
...
```{r}
add_cd4_form <- as.formula(paste0("unsuccessful ~", (paste(boot_vars_no_hiv, collapse = "+")), "+ cd4_lt200_all"))

add_cd4 <-  lrm(add_cd4_form, data=data_to_analyze)
...
```{r}
add_vl_form <- as.formula(paste0("unsuccessful ~", (paste(boot_vars_no_hiv, collapse = "+")), "+ vl_gt200_all"))

add_vl <-  lrm(add_vl_form, data=data_to_analyze) 
...
add_cd4 <-  lrm(add_cd4_form, data=cc_analaysis)
...
add_vl <-  lrm(add_vl_form, data=cc_analaysis) 
...
boot_model_dc <- decision_curve(boot_form_70, 
                                data = mi_sum,
                                thresholds = seq(0, .7, by = .05),
                                study.design = "cohort",
                                bootstraps = 200)

jpeg(here("Manuscript", "images", "decision_curve.jpeg"), width=600, height=450)
plot_decision_curve(boot_model_dc,
                    curve.names = "Risk model",
                    ylim = c(0, 1),
...
# The model is never worse than treating patients
# Number of unnecessary interventions is calculated as [net benefit of the model - net benefit of treat all]/[p_t/(1-p_t)] *100

# Calculates number of avoidable interventions of using prediction model vs. treat all 
net_benefit <- boot_model_dc$derived.data %>% 
  select(thresholds, NB, sNB, model) %>% 
  mutate(model = ifelse(grepl("unsuccessful", model), "pred_model", model)) %>% 
  arrange(thresholds) %>% 
  pivot_wider(names_from = model,
              id_cols = thresholds,
              values_from = c("NB", "sNB")
              ) %>% 
  mutate(nb_dif = NB_pred_model - NB_All,
         snb_dif = sNB_pred_model - sNB_All,
         red_avoid_int = nb_dif/(thresholds/(1-thresholds))*100,
         s_red_avoid_int = snb_dif/(thresholds/(1-thresholds))*100)

# If we decide to treat only people with predicted outcome risk >0.2, this will have a net benefit (more true positives without expense of false negatives) over treat all of y-axis value
# Alternatively, prediction model will have a net benefit over treat none at risk thresholds where model line > treat none line
# Areas on the x-axis where the lower bound of the 95% CI is higher than the “treatment for all” line (i.e., threshold probability > 5%) offers a benefit over treating all individuals (improvement of false positives - specificity - without loss of true positives - sensitivity). 
# Areas on the x-axis where the lower bound of the 95% CI does not cross 0 (threshold probability < 41%) illustrate settings in which use of the clinical risk score offers a  benefit compared to treating none (improvement of true positive - sensitivity - without increase in false positive)


summary(boot_model_dc)
```
